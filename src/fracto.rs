...
fn kmax_color_model(cycle: &History, cycle_max: &History) -> f64 {
    ...
    (cycle.kmax.a.unwrap() / cycle_max.kmax.a.unwrap()).powf(3.0).clamp(0.45, 0.65)
...
pub fn write_svg_pseudo_image(history: &[History], frame: &ImageData) {
    ...
    let a_max = history[history.len() - 1].geometry.a.as_ref().unwrap().length;
    let a_init = history[0].geometry.a.as_ref().unwrap().length;
    ...
    let cycle_max = history.iter().max_by(|h0, h| NonNan::new(h0.kmax.a.unwrap()).cmp(&NonNan::new(h.kmax.a.unwrap()))).unwrap();
    println!("cycle max: {:?}", cycle_max);
    
    // plot each cycle in the history
    for cycle in history.iter() {
        //        println!("{:?}", cycle);
        let striation = cycle.geometry.a.as_ref().unwrap().length - a_prev; // the width of the current striation
        a_prev = cycle.geometry.a.as_ref().unwrap().length;
        ...
        let y_cur = f64::from(frame.ysize) * ((cycle.geometry.a.as_ref().unwrap().length - a_init) / (a_max - a_init));
...
mod tests {
    use super::*;
    use crate::beta::BetaResult;
    use crate::cycle::Cycle;
    use crate::grow::{CrackGeometry, CrackFront, CrackLengthDelta, StressIntensity, StressIntensityDelta};
    use crate::tag::Tag;
    use std::path::Path;
    use std::fs;
    ...
    fn test_make_pseudo_image() {
        ...
        let hist = |a, block, peak| History {
            ...
            kmax: StressIntensity{ a: Some(peak), c: None},
            kmin: StressIntensity::default(),
            dk: StressIntensityDelta{ a: Some(peak), c: None},
            beta: BetaResult {
                a: Some(1.0),
                c: None,
            },
            length_deltas: CrackLengthDelta{ a: Some(da), c: None},
