/// Tables with spline interpolation using the cubic_splines library

use log::debug;

use cubic_splines::{Spline, BoundaryCondition};
use crate::table::nearest;
...
impl PairTable {
    pub fn new(columns: Vec<f64>,
        ...
        for col in 0..columns.len() {
            ...
            for row in 0..rows[col].len() {
                points.push((rows[col][row], values[col][row]));
            }

            splines.push(Spline::new(points, BoundaryCondition::Natural));
        }
    ...
    pub fn interp(&self, row: f64, column: f64) -> f64 {
        ...
        let row_s = self.splines[col_low].eval(row_limited);
        ...
        if self.columns.len() > 1 {
            ...
            let row_e = self.splines[col_low + 1].eval(row_limited);
