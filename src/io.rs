...
pub fn read_table(filename: &str) -> Vec<Vec<f64>> {
    ...
    let mut file = match File::open(path) {
        Err(why) => {
            error!("Error: could not open the file '{}': {}.", display, why);
            std::process::exit(1)
        }
        Ok(file) => file,
    };
    ...
    match file.read_to_string(&mut s) {
        Err(why) => {
            error!("Error: could not read {}: {}.", display, why);
            std::process::exit(1)
        }
        Ok(_) => true,
    };
...
pub fn read_afgrow_cycles(cyclesfile: &str) -> Vec<Cycle<Tag>> {
    ...
    for c in rcycles {
        // Skip blank entries (commonly at the end of a file)
        if c.len() != 3 {
            continue;
        }
...
pub fn write_cycles(file: &str, cycles: &[Cycle<Tag>]) {
    ...
    writeln!(file, "0 {}", cycles.len()).unwrap();

    for &Cycle { min, max } in cycles.iter() {
        let result = writeln!(&mut file, "{:?} {:?} 1", max.value, min.value);
        match result {
            Ok(_result) => (),
            Err(err) => error!(
