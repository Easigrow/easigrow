...
pub fn match_crack(main_options: &options::EasiOptions, factors: &mut [f64]) -> f64 {
    ...
    let data = Data {
        options: all_options,
        params: main_options.params.values().cloned().collect(),
    };
    ...
    // This line was changed with almost no understanding of the algorithm.
    // nevalf and nevaldf have been set to 'm' purely on intuition and should
    // almost certainly be set to something else.
    let mut f = rgsl::MultiFitFunctionFdf::new(m, n, m, m);
